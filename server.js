...
app.get('/health',(_,res)=>res.json({ok:true,name:'5 Tokes Multiplayer',version:'0.6.9',rooms:rooms.size,players:[...rooms.values()].reduce((n,r)=>n+r.players.filter(p=>p.connected&&!p.isBot).length,0),computerPlayers:[...rooms.values()].reduce((n,r)=>n+r.players.filter(p=>p.isBot).length,0),leaderboardEntries:leaderboard.entries(500).length}));
...
function scheduleComputer(r){
  ...
  if(!r.started||!bot?.isBot||botTimers.has(r.code))return;
  const delay=r.turnStage==='draw'?750:900;
  const timer=setTimeout(()=>{
    botTimers.delete(r.code);
    if(rooms.get(r.code)!==r||!r.started||!r.currentPlayer()?.isBot)return;
    try{r.computerStep();emitRoom(r);}catch(e){console.error(`[cpu ${r.code}]`,e);r.message='The computer player hit an error. Please leave the room and try again.';emitRoom(r);}
  },delay);
...
function exitFinishedRoom(s,reason='left'){
  ...
  const empty=r.leave(s.data.playerId);clearSocketRoom(s,code,reason);if(empty||!hasHumanPlayers(r))rooms.delete(code);else emitRoom(r);
}
io.on('connection',s=>{
  ...
  s.on('createComputerGame',({name,avatarKey,profileId}={})=>{try{
    const c=roomCode(),token=crypto.randomUUID(),r=new GameRoom(c,s.id,cleanName(name),token,cleanAvatar(avatarKey),cleanProfileId(profileId));
    const human=r.players[0];r.addComputerPlayer('Royal Toker CPU','highstakes');rooms.set(c,r);bind(s,r,human,false);r.start(human.id);r.message='Solo match started against Royal Toker CPU.';emitRoom(r);
  }catch(e){fail(s,e);}});
  ...
  s.on('forfeitGame',()=>{try{const r=room(),code=r.code;const empty=r.forfeit(s.data.playerId);clearSocketRoom(s,code,'forfeit');emitRoom(r);if(empty||!hasHumanPlayers(r))rooms.delete(code);}catch(e){fail(s,e);}});
