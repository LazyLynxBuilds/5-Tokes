...
app.get('/health',(_,res)=>res.json({ok:true,name:'5 Tokes Multiplayer',version:'0.6.6',rooms:rooms.size,players:[...rooms.values()].reduce((n,r)=>n+r.players.filter(p=>p.connected).length,0)}));
...
function bind(s,r,p){s.join(r.code);s.data.roomCode=r.code;s.data.playerId=p.id;s.emit('session',{roomCode:r.code,playerToken:p.sessionToken,playerId:p.id,name:p.name,avatarKey:p.avatarKey});emitRoom(r);}
...
io.on('connection',s=>{
  s.on('createRoom',({name,avatarKey}={})=>{try{const c=roomCode(),token=crypto.randomUUID(),r=new GameRoom(c,s.id,cleanName(name),token,cleanAvatar(avatarKey));rooms.set(c,r);bind(s,r,r.players[0]);}catch(e){fail(s,e);}});
  s.on('joinRoom',({name,roomCode:rc,avatarKey}={})=>{try{const c=String(rc||'').toUpperCase().trim(),r=rooms.get(c);if(!r)throw new Error('Room not found.');const p=r.addPlayer(s.id,cleanName(name),crypto.randomUUID(),cleanAvatar(avatarKey));bind(s,r,p);}catch(e){fail(s,e);}});
