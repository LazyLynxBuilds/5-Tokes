...
function newPlayer(name,socketId,token,avatarKey='jahbuddy'){return {id:crypto.randomUUID(),sessionToken:token||crypto.randomUUID(),socketId,name,avatarKey,hand:[],score:0,connected:true,finalDone:false,roundPenalty:null};}


class GameRoom{
  constructor(code,hostSocketId,hostName,sessionToken,avatarKey='jahbuddy'){const p=newPlayer(hostName,hostSocketId,sessionToken,avatarKey);this.code=code;this.players=[p];this.hostId=p.id;this.started=false;this.roundRank=3;this.dealerIndex=0;this.turnIndex=0;this.turnStage='draw';this.drawPile=[];this.discardPile=[];this.outPlayerId=null;this.roundEnding=false;this.winnerIds=[];this.outMelds=[];this.message='Waiting for players.';this.lastActivity=Date.now();}
  touch(){this.lastActivity=Date.now();}
  addPlayer(socketId,name,token,avatarKey='jahbuddy'){if(this.started)throw new Error('Game already started.');if(this.players.length>=7)throw new Error('Room is full (7 players max).');const p=newPlayer(name,socketId,token,avatarKey);this.players.push(p);this.touch();return p;}
