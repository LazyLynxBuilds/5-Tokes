...
class GameRoom{
  ...
  discard(id,cardIdToDiscard){
    this.ensureTurn(id,'discard');const p=this.currentPlayer(),playerName=p.name,idx=p.hand.findIndex(c=>c.id===cardIdToDiscard);if(idx<0)throw new Error('Card not found in your hand.');const [card]=p.hand.splice(idx,1);this.discardPile.push(card);if(this.roundEnding&&id!==this.outPlayerId){p.finalDone=true;p.roundPenalty=minimumRemainingScore(p.hand,this.roundRank);}const beforeRound=this.roundRank;this.touch();this.advanceTurn();const sameRound=this.started&&this.roundRank===beforeRound,next=sameRound?this.currentPlayer():null;this.announce('discard',{playerId:id,playerName,card:publicCard(card),nextPlayerId:next?.id,nextPlayerName:next?.name,roundComplete:!sameRound});if(sameRound)this.message=`${playerName} discarded. ${next.name}'s turn.`;
  }
  goOut(id,discardId,melds){
    this.ensureTurn(id,'discard');const p=this.currentPlayer(),idx=p.hand.findIndex(c=>c.id===discardId);if(idx<0)throw new Error('Choose a card to discard.');if(!validateMeldLayout(p.hand,discardId,melds,this.roundRank))throw new Error('Build valid melds (Meld Stashes) using every card except your selected discard before going down.');const before=new Map(p.hand.map(c=>[c.id,c])),finalGroups=melds.map(g=>g.map(cid=>before.get(cid)).filter(Boolean));const [card]=p.hand.splice(idx,1);this.discardPile.push(card);p.roundPenalty=0;p.finalDone=true;
    const wasRoundEnding=this.roundEnding;
    if(!wasRoundEnding){this.outMelds=finalGroups.map(g=>g.map(publicCard));this.outPlayerId=id;this.roundEnding=true;this.message=`${p.name} went down! Everyone else gets one final turn.`;}
    const beforeRound=this.roundRank;this.touch();this.advanceTurn();const sameRound=this.started&&this.roundRank===beforeRound,next=sameRound?this.currentPlayer():null;this.announce(wasRoundEnding?'finalGoDown':'goDown',{playerId:id,playerName:p.name,card:publicCard(card),melds:finalGroups.map(g=>g.map(publicCard)),nextPlayerId:next?.id,nextPlayerName:next?.name,roundComplete:!sameRound});if(wasRoundEnding&&sameRound)this.message=`${p.name} went down on the final turn. ${next.name}'s turn.`;
  }
  computerStep(){
    const bot=this.currentPlayer();if(!this.started||!bot?.isBot)return {acted:false};
